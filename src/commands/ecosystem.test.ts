...
describe("getInstalledVersion", () => {
	test("returns a version string for git", async () => {
		const version = await getInstalledVersion("git");
		expect(version).not.toBeNull();
		expect(version).toMatch(/\d+\.\d+\.\d+/);
	});

	test("returns null for nonexistent tool", async () => {
		const version = await getInstalledVersion("nonexistent-tool-xyz-999");
